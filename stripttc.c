...
/*
 * This is stripttc.c from contrib/fonttools/stripttc.c in the FontForge
 * github repo, modified to always write its outputs in the current
 * directory.
 */
...
static int handlefont(char *filename,int which,FILE *ttc,int offset) {
    char *scratch, *outfile, *ext;
    FILE *ttf;
    int i, cnt, *offsets, *lengths, head, tag, pos, headpos;

    scratch = strdup(filename);
    strcpy(scratch, basename(scratch));
    ext = strrchr(scratch, '.');
    if (ext != NULL) *ext = '\0';
    cnt = strlen(scratch) + strlen("_XX.ttf") + 1;
    outfile = malloc(cnt);
    snprintf(outfile, cnt, "%s_%02d.ttf", scratch, which);
