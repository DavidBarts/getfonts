/*
 * This is showttf.c from contrib/fonttools/showttf.c in the FontForge
 * github repo, hacked to only show the PostScript name of the font.
 */
...
/* End guesses */
        ...
        psname = id == 6 ? malloc(strlen + 1) : NULL;
        fseek(util,info->copyright_start+taboff+stroff,SEEK_SET);
        for ( j=0; j<strlen; ++j ) {
            int ch = getc(util);
            if (psname != NULL) psname[j] = ch;
            if ( ch<' ' ) {
                putchar('^');
                putchar(ch+'@');
            } else if ( ch=='\177' ) {
...
static char *readit(FILE *ttf, FILE *util) {
    struct ttfinfo info;
    int i, pos;
...
int main(int argc, char **argv) {
    ...
    for ( i=1; i<argc; ++i ) {
        if ( *argv[i]=='-' ) {
            ...
            if ((fontname = readit(ttf,util)) == NULL || !strlen(fontname)) {
